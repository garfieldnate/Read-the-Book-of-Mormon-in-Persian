...
Read the cleaned source at `01/normalized.txt` while working through this guide. A few artifacts remain that the normalizer can't fix because they're typographic, not orthographic:
...
Long vowels `ā ī ū`, short `a e o`. Consonants: `š ž x ʿ ʾ č j`. Ezafe `-e` / `-ye`. Object marker `-rā`. Indefinite `-ī`. Possessive suffixes `-am -at -aš -mān -tān -šān`.

---

## Vocabulary

Organized in **order of first appearance**, grouped by verse. A word appears in the verse where you first encounter it; inflected forms of a verb already introduced (e.g. a new past-tense of a verb you saw earlier) are not re-listed. Proper nouns are mixed in where they first appear, tagged `[proper]`. Verbs include infinitive and present stem. Line numbers reference `01/normalized.txt`.
...
Twelve points. Every example is taken verbatim from `01/normalized.txt` (line numbers cited) and shown in three rows: Persian, transcription, English.
