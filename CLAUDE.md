...
`lexicon.json` (repo root) is the source of truth for per-word transliterations (`tl`, normalized; a list for homographs) and proper-name translations (`name`). `build_site.py`'s `lint_lexicon` validates every token against it and flags mismatches or words missing from it. When adding vocabulary, add it to `lexicon.json`; don't let translits drift chapter-to-chapter. Contextual definitions/glosses are not pinned.
