...
- **When to add `*Etym*`** (etymology / morpheme breakdown — included only when the answer is interesting):
  ...
  **Cross-linking affixes to `word_formation.html`**: whenever an affix you mention in `*Etym*`, `*Family*`, or `*Forms*` is covered by `study_guide/word_formation.md`, make the affix text itself a Markdown link to the relevant anchor on `../word_formation.html`. Example: `` [`-ی`](../word_formation.html#the-abstract-noun-suffix-g) `` or `` [`بر-`](../word_formation.html#directional-prefixes-bar-and-foru) ``. Link the morpheme text directly — do not add a separate parenthetical label. To find an anchor, build the site and inspect the `id=` attributes in `_site/study_guide/word_formation.html`, or compute the slug with `re.sub(r"[^a-z0-9]+", "-", heading.lower()).strip("-")` applied to the heading text (Persian/extended-Latin characters collapse to `-`, only ASCII a–z and 0–9 survive). Do **not** link ezafe (`-e`), the object marker (`-rā`), the plural suffix (`-hā`), the comparative (`-tar`), possessive clitics (`-am/-at/-aš/-mān/-tān/-šān`), or any morpheme not documented in `word_formation.md`.
